...
class PPOTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(
        ...
        if self.config.RL.DDPPO.get("pretrained_map_encoder", False):
            ...
            if "resnet" in self.config.RL.PPO.get("cnn_type", "cnn_ans_test_2"):
                prefix = "encoder."
        ...
        if not self.config.RL.DDPPO.get("train_map_encoder", True):
            if "resnet" in self.config.RL.PPO.get("cnn_type", "cnn_ans_test_2"):
                ...
                for param in self.actor_critic.net.visual_fc.parameters():
                    param.requires_grad_(False)
                if self.config.RL.PPO.get("cnn_type", "cnn_ans_test_2") == "resnet18":
        ...
        if self.config.RL.POLICY.name == "PointNavSplitNetPolicy":
            if self.config.RL.SPLITNET.pretrained_encoder:
                pretrained_state = torch.load(
                    self.config.RL.SPLITNET.pretrained_weights,
                    map_location="cpu",
                )
    ...
    def _init_train(self):
        ...
        if (
            ...
            if "CONTEXT_WAYPOINT_SENSOR" in str(self.config.TASK_CONFIG.TASK.SENSORS):
                context_shape = (2,)
                obs_space = spaces.Dict(
                    {
        ...
        if self.config.RL.POLICY.name == "PointNavContextSMTPolicy":
            self.rollouts = RolloutStorage(
                ...
                external_memory_size=ppo_cfg.SCENE_MEMORY_TRANSFORMER.memory_size,
                external_memory_capacity=ppo_cfg.SCENE_MEMORY_TRANSFORMER.memory_size,
                external_memory_dim=self.actor_critic.net.memory_dim,
    ...
    def _compute_actions_and_step_envs(self, buffer_index: int = 0):
        ...
        with torch.no_grad():
            ...
            (
                ...
                step_batch["masks"],
                step_batch["external_memory"],
                step_batch["external_memory_masks"],
            )
        ...
        self.rollouts.insert(
            ...
            value_preds=values,
            next_memory_features=external_memory_features,
        ...
        while len(stats_episodes) < number_of_eval_episodes and self.envs.num_envs > 0:
            ...
            with torch.no_grad():
                (
                    ...
                    prev_actions,
                    not_done_masks,
                    test_em.ext_memory[:, 0] if use_external_memory else None,
                    test_em.ext_masks if use_external_memory else None,
            ...
            if use_external_memory:
                test_em.insert(test_memory_features, not_done_masks)
            rewards = torch.tensor(
            ...
            for i in range(n_envs):
                ...
                if not not_done_masks[i].item():
                    ...
                    all_episode_stats[current_episodes[i].episode_id] = episode_stats
