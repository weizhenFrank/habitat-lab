...
def get_offset(pred, action):
    ...
    if action == 'f':
        trans_amt = 0.25
        offset = np.zeros_like(pred)
        offset[:, 0] = np.subtract(np.abs(pred[:, 0]), trans_amt)
        offset[:, 1] = pred[:, 1]
        offset[:, 2] = pred[:, 2]
    else:
        if action == 'l':
...
if __name__ == "__main__":
    ...
    for filename in glob.iglob(args.ckpt_dir + "**/*.pt", recursive=True):
        ...
        print('Mean L_offset: ')
        print(np.mean(L_offset, axis=0))
        print('Std L_offset: ')
        print(np.std(L_offset, axis=0))
        print('Mean R_offset: ')
        print(np.mean(R_offset, axis=0))
        print('Std R_offset: ')
        print(np.std(R_offset, axis=0))
