...
class Raibert_controller:
    def __init__(
        ...
        if "A1" in robot:
            ...
            self.kinematics_solver = quadruped_kinematics_solver(
                **aliengo_config
            )
        ...
        if des_body_ori is None:
            self.des_body_ori = np.array(
                [0, 0, 0]
            )  # this is for des orientation at each timestep
        else:
    ...
    def set_control_params(self, state):
        ...
        self.swing_start_foot_pos_robot = (
            self.kinematics_solver.forward_kinematics_robot(state["j_pos"])
        )

        self.swing_start_foot_pos_world = (
            self.kinematics_solver.robot_frame_to_world_robot(
                state["base_ori_euler"] * 0, self.swing_start_foot_pos_robot
            )
        )
    ...
    def plan_latent_action(self, state, target_speed, target_ori=0.0):
        ...
        speed_term = (
            self.num_timestep_per_HL_action
    ...
    def update_latent_action(self, state, latent_action):
        ...
        self.final_des_body_ori[2] = (
            self.last_com_ori[2] + self.latent_action[-1]
        )
        target_delta_xy = np.zeros(self.num_legs * 3)
        for i in range(self.num_legs):
            if i in self.swing_set:
                target_delta_xy[3 * i] = (
                    self.init_foot_pos[i][0]
                    + self.latent_action[0]
                    - self.swing_start_foot_pos_robot[3 * i]
                )
                target_delta_xy[3 * i + 1] = (
                    self.init_foot_pos[i][1]
    ...
    def _get_action(self, state, phase):
        ...
        self.des_body_ori[2] = (
            self.final_des_body_ori[2] - self.last_com_ori[2]
        ) * phase + self.last_com_ori[2]
        # this seems to be designed only when walking on a flat ground
        des_foot_height_delta = self.leg_clearance * math.sin(
            math.pi * phase + EPSILON
        )

        for i in range(self.num_legs):
            des_single_foot_pos = np.zeros(3)
            if i in self.swing_set:
                des_single_foot_pos[2] = (
                    des_foot_height_delta - self.standing_height
                )
            else:
                des_single_foot_pos[2] = (
                    des_foot_height_delta * 0.15 - self.standing_height
                )

            des_single_foot_pos[0] = (
                self.target_delta_xyz[3 * i] * phase
                + self.swing_start_foot_pos_robot[3 * i]
            )
            des_single_foot_pos[1] = (
                self.target_delta_xyz[3 * i + 1] * phase
                + self.swing_start_foot_pos_robot[3 * i + 1]
            )
            self.des_foot_position_com = np.append(
                self.des_foot_position_com, des_single_foot_pos
            )
...
class Raibert_controller_turn(Raibert_controller):
    def __init__(
        ...
        Raibert_controller.__init__(
            self,
            robot=robot,
            num_timestep_per_HL_action=num_timestep_per_HL_action,
            target=target,
            speed_gain=speed_gain,
            des_body_ori=des_body_ori,
    ...
    def plan_latent_action(self, state, target_speed, target_ang_vel=0.0):
        ...
        mult_factor = (
            0.5 * self.num_timestep_per_HL_action / self.control_frequency
        )

        acceleration_term = (
            self.speed_gain * (self.target_speed - current_speed)
            + current_speed * mult_factor
        )
        orientation_speed_term = (
            self.speed_gain * (target_ang_vel - current_yaw_rate)
            + mult_factor * current_yaw_rate
        )
    ...
    def update_latent_action(self, state, latent_action):
        ...
        self.last_com_ori = np.array(
            [0, 0, 0]
        )  # np.array(state['base_ori_euler'])
        ...
        self.target_delta_xyz_world = (
            self.kinematics_solver.robot_frame_to_world_robot(
                self.last_com_ori, target_delta_xy
            )
        )
    ...
    def _get_action(self, state, phase):
        self.des_foot_position_com = np.array([])
        self.des_body_ori = (
            self.final_des_body_ori - self.last_com_ori
        ) * phase + self.last_com_ori
        ...
        for i in range(self.num_legs):
            ...
            if i in self.swing_set:
                ...
                des_single_foot_pos[2] = (
                    0.1 * des_foot_height_delta - self.standing_height
                )

            des_single_foot_pos[0] = (
                self.target_delta_xyz_world[3 * i] * phase
                + self.swing_start_foot_pos_world[3 * i]
            )
            des_single_foot_pos[1] = (
                self.target_delta_xyz_world[3 * i + 1] * phase
            ...
            self.des_foot_position_com = np.append(
                ...
                self.kinematics_solver.world_frame_to_robot_leg(
                    self.des_body_ori, des_single_foot_pos
                ),
            )
    ...
    def update_latent_action(self, state, latent_action):
        ...
        for i in range(self.num_legs):
            ...
            if i in self.swing_set:
                ...
                target_delta_xy[3 * i] = (
                    self.latent_action[0]
                    + rad * self.target_ang_vel * math.sin(theta) * mult_factor
                )
                target_delta_xy[3 * i + 1] = (
                    self.latent_action[1]
                    + rad * self.target_ang_vel * math.cos(theta) * mult_factor
                )
            else:
                target_delta_xy[3 * i] = (
                    -self.latent_action[0]
