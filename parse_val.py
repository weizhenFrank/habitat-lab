...
class Stats(object):
    ...
    def calculate_stats(self, csv, split, eps=None, eps_ignore=None):
        ...
        df_split = df[[ep_id in SPLIT for ep_id in df.ep_id]]
        ...
        stats["collisions_stats"] = self.mean_confidence_interval(
            np.array(df_split.collisions)
        )
    ...
    def get_valid_csvs(
        ...
        for csv in sorted(glob.glob(os.path.join(agent_dir, "*csv")), key=ckpt_to_idx):
