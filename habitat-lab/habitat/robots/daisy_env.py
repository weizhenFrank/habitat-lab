...
from .utils import get_rpy, rotate_vector_3d
...
class Daisy:
    ...
    def set_up_continuous_action_space(self):
        self.high_level_lin_action_space = gym.spaces.Box(
            shape=(self.high_level_action_dim,),
            low=-self.linear_velocity,
            high=self.linear_velocity,
            dtype=np.float32,
        )
        self.high_level_ang_action_space = gym.spaces.Box(
            shape=(1,),
    ...
    def calc_state(self):
        ...
        if finite_diff:
            if prev_state is None:
                base_velocity = (
                    mn.Vector3()
                )  # self.sim.get_articulated_link_angular_velocity(self.robot_id, 0)
                frame_pos = np.zeros((3))
                base_angular_velocity_euler = (
                    mn.Vector3()
                )  # self.sim.get_articulated_link_angular_velocity(self.robot_id, 0)
            else:
                ...
                base_angular_velocity_euler = (
                    base_orientation_euler - prev_state["base_ori_euler"]
                ) / self.dt
        else:
        ...
        return {
            ...
            "base_ang_vel": rotate_vector_3d(
                base_angular_velocity_euler, *base_orientation_euler
            ),
        ...
        for n, j in enumerate(self.ordered_joints):
            ...
            if self.control == "velocity":
                j.set_motor_velocity(
                    self.velocity_coef * j.max_velocity * float(np.clip(a, -1, +1))
                )
                ...
                j.set_motor_torque(
                    self.torque_coef * j.max_torque * float(np.clip(a, -1, +1))
                )
            else:
