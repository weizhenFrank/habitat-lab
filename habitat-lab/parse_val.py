...
def main():
    ...
    best_csv = S.get_best_checkpoint(args.agent_dir, args.type, split='all', ckpt=args.checkpoint, step_limit=args.limit)
    pprint(f'Best checkpoint:\n {best_csv}')
    all_stats = S.calculate_stats(best_csv, split='all')
    pprint('Best checkpoint stats: ')
    print('Success, SPL, # Steps, # Collisions, Dist2Goal')
...
class Stats(object):
    def return_successful_eps(self, csv, split):
        ...
        ret = [episode_id for episode_id, success in zip(df.id, df.success) if float(success)==1.0 and episode_id in SPLIT]
        return ret
    def return_unsuccessful_eps(self, csv, split):
        SPLIT = list(range(NUM_EPISODES))
        with open(csv) as f:
            df = pandas.read_csv(f)
        ret = [episode_id for episode_id, success in zip(df.id, df.success) if float(success)==0.0 and episode_id in SPLIT]
        return ret
    '''
    ...
    def calculate_stats(self, csv, split, eps=None, eps_ignore=None):
        ...
        stats['success_stats'] = self.mean_confidence_interval(np.array(df_split.success))
        stats['episode_distance_stats'] = self.mean_confidence_interval(np.array(df_split.episode_distance))
        stats['success_episode_distance_stats'] = self.mean_confidence_interval(np.array(df_split[df_split.success == 1.0].episode_distance))
        stats['fail_episode_distance_stats'] = self.mean_confidence_interval(np.array(df_split[df_split.success == 0.0].episode_distance))
        stats['num_actions_stats'] = self.mean_confidence_interval(np.array(df_split.steps))
        return stats
    '''
    ...
    def get_best_checkpoint(self, agent_dir, ty, split='all', ckpt=-1, step_limit=2e8):
        ...
        for csv in tqdm.tqdm(csvs):
            ...
            try:
                ...
                pprint(f'[ERROR]: {csv} which does not have {NUM_EPISODES} num episodes. Skipping.')
