...
class PhysicsManager {
  ...
  */
  ...
  Magnum::Vector3 getArticulatedLinkAngularVelocity(int objectId, int linkId) {
    CHECK(existingArticulatedObjects_.count(objectId));
    return existingArticulatedObjects_.at(objectId)->getArticulatedLinkAngularVelocity(
      linkId);
  };

  Magnum::Vector3 getArticulatedLinkLinearVelocity(int objectId, int linkId) {
    CHECK(existingArticulatedObjects_.count(objectId));
    return existingArticulatedObjects_.at(objectId)->getArticulatedLinkLinearVelocity(
      linkId);
  };
  
  std::vector<Magnum::Vector3> getArticulatedLinkVelocity(int objectId, int linkId) {
    CHECK(existingArticulatedObjects_.count(objectId));
    return existingArticulatedObjects_.at(objectId)->getArticulatedLinkVelocity(
      linkId);
  };
