...
parser.add_argument(
    "-vl", "--use_visual_loss", default=False, action="store_true"
)
parser.add_argument(
    "-so", "--separate_optimizers", default=False, action="store_true"
)
parser.add_argument(
    "-pe", "--pretrained-encoder", default=False, action="store_true"
)
# Evaluation
...
EXP_YAML = (
    "habitat_baselines/config/pointnav/ddppo_pointnav_spot_splitnet_train.yaml"
)
EVAL_EXP_YAML = (
    "habitat_baselines/config/pointnav/ddppo_pointnav_spot_splitnet_eval.yaml"
)
TASK_YAML = "configs/tasks/pointnav_spot_splitnet_train.yaml"
EVAL_YAML = "configs/tasks/pointnav_spot_splitnet_eval.yaml"
...
if not args.eval:
    ...
    for idx, i in enumerate(exp_yaml_data):
        ...
        elif i.startswith("    separate_optimizers:"):
            if args.separate_optimizers:
                exp_yaml_data[idx] = "    separate_optimizers: True"
            else:
                exp_yaml_data[idx] = "    separate_optimizers: False"
        elif i.startswith("    use_visual_loss:"):
            if args.use_visual_loss:
                exp_yaml_data[idx] = "    use_visual_loss: True"
            else:
                exp_yaml_data[idx] = "    use_visual_loss: False"
    ...
    if args.debug:
        ...
        slurm_data = slurm_data.replace("GPUS", str(args.num_gpus))
...
# Evaluation
    ...
    for idx, i in enumerate(exp_yaml_data):
        if i.startswith("BASE_TASK_CONFIG_PATH:"):
            ...
            tb_dir = (
                "tb_eval_hm3d_gibson_{}".format(robots_underscore)
                if args.ckpt == -1
                else "tb_eval_hm3d_gibson_{}_ckpt={}".format(
                    robots_underscore, args.ckpt
                )
            )
            if args.video:
                tb_dir = (
                    "tb_eval_hm3d_gibson_{}_video".format(robots_underscore)
                    if args.ckpt == -1
                    else "tb_eval_hm3d_gibson_{}_ckpt={}_video".format(
                        robots_underscore, args.ckpt
                    )
                )
            ...
            txt_dir = (
                "txts_eval_hm3d_gibson_{}".format(robots_underscore)
                if args.ckpt == -1
                else "txts_eval_hm3d_gibson_{}_ckpt={}".format(
                    robots_underscore, args.ckpt
                )
            )
            exp_yaml_data[idx] = "TXT_DIR:            '{}'".format(
    ...
    with open(EVAL_SLURM_TEMPLATE) as f:
        ...
        slurm_data = slurm_data.replace(
            "TEMPLATE",
            f"eval_{experiment_name}_hm3d_gibson_{robots_underscore}",
        )
        slurm_data = slurm_data.replace("HABITAT_REPO_PATH", HABITAT_LAB)
    ...
    slurm_path = os.path.join(
        dst_dir,
        "eval_bash_files",
        experiment_name + "_hm3d_gibson_" + robots_underscore + ".sh",
    )
    with open(slurm_path, "w") as f:
        f.write(slurm_data)
    print("Generated slurm job: " + slurm_path)
